/**
 * Heroine Evolution Sprite Generator V3 - MATCHING HERO QUALITY
 * Regenerates 40 HIGHLY DETAILED female character sprites
 * Using IDENTICAL prompts as the hero version, just with female character base
 *
 * This ensures visual consistency between hero and heroine sprites
 */
...
// Character continuity - FEMALE VERSION (matching male style exactly)
const CHARACTER_BASE = `athletic female heroine with short brown hair and brown eyes`;

// 40 Stage Evolution - IDENTICAL TO HERO VERSION
...
// Generation parameters (matching hero V3 exactly)
...
// Negative prompt - FEMALE VERSION (exclude male references)
...
different person, male, man, boy, long hair, blonde hair, red hair, black hair, different hair color, different face, different character,
oversexualized, revealing armor, bikini armor, high heels, skimpy outfit`;
...
async function generateAllStages() {
  console.log('🚀 Starting Heroine Evolution V3 Regeneration - MATCHING HERO QUALITY');
  ...
  for (let i = 0; i < EVOLUTION_STAGES.length; i++) {
    ...
    const filename = `heroine_v3_stage_${stage.level}_${stage.name.toLowerCase().replace(/\s+/g, '_')}.png`;
  ...
  console.log('🎉 Heroine Evolution V3 regeneration complete!\n');

  // Save metadata
  const metadata = {
    version: 'V3-Heroine-Regen',
    ...
    stages: EVOLUTION_STAGES.map(s => ({
      level: s.level,
      name: s.name,
      filename: `heroine_v3_stage_${s.level}_${s.name.toLowerCase().replace(/\s+/g, '_')}.png`
    }))
  };

  const metadataPath = path.join(__dirname, '..', 'data', 'heroineEvolutionV3Metadata.json');
  const metadataDir = path.dirname(metadataPath);
  if (!fs.existsSync(metadataDir)) {
    fs.mkdirSync(metadataDir, { recursive: true });
  }
  fs.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2));
  console.log('📄 Metadata saved to data/heroineEvolutionV3Metadata.json');
}
