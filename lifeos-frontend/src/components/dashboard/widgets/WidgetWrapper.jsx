...
export default function WidgetWrapper({ widgetId, children, className = '' }) {
  ...
  // Get tour attribute for this widget
  const tourAttribute = WIDGET_TOUR_MAP[widgetId];
        ...
        <>
          ...
          <div
            className={`
              ...
              ${isDragging
                ? 'bg-primary-500/40 backdrop-blur-sm'
                : isHovered
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <div className={`
                ...
                ${isDragging ? 'bg-primary-500/50 scale-110' : 'bg-transparent'}
              `}>
                <GripVertical className={`
                  w-4 h-4 transition-colors duration-200
                  ${isDragging ? 'text-text-primary' : 'text-primary-400'}
                `} />
              </div>
              <span className={`
                text-xs font-medium transition-all duration-200
                ${isDragging ? 'text-text-primary' : 'text-primary-300'}
              `}>
            ...
            >
              <button
                ...
                className="p-1.5 hover:bg-error/30 rounded-lg transition-all duration-200 group/remove cursor-pointer z-50"
                title="Remove widget"
                type="button"
              >
                <X className="w-3.5 h-3.5 text-text-secondary group-hover/remove:text-error transition-colors pointer-events-none" />
              </button>
            </div>
          </div>
          ...
          <div className={`
            ...
            ${isDragging
              ? 'border-2 border-primary-500 shadow-lg shadow-primary'
              : isHovered
                ? 'border-2 border-dashed border-primary-500/50'
                : 'border-2 border-dashed border-primary-500/20'
            }
          `} />
